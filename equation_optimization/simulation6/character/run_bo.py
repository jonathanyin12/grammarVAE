...
diff_model = equation_vae.EquationGrammarModel("../../../pretrained/eq_vae_grammar_h100_c234_L25_E50_batchB.hdf5")
...
def decode_from_latent_space(latent_points, grammar_model):
    ...
    for i in range(decode_attempts):
        ...
        for j in range(latent_points.shape[ 0 ]):
            ...
            try:
                tokens = equation_vae.tokenize(smile)
                parse = next(diff_model._parser.parse(tokens))
                res = eval(smile)
...
X = np.loadtxt('../../latent_features_and_targets_character/latent_features_eq.txt')
y = np.loadtxt('../../latent_features_and_targets_character/targets_eq.txt')
y = y.reshape((-1, 1))
...
for iteration in range(5):
    ...
    grammar_weights = "../../../pretrained/eq_vae_str_h100_c234_L25_E50_batchB.hdf5"
    grammar_model = equation_vae.EquationCharacterModel(grammar_weights,latent_rep_size=25)
    ...
    x = np.loadtxt('../../latent_features_and_targets_character/x_eq.txt')
    yT = np.loadtxt('../../latent_features_and_targets_character/true_y_eq.txt')
