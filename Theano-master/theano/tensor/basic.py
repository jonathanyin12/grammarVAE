...
class Reshape(Op):
    ...
    def infer_shape(self, node, ishapes):
        ...
        if isinstance(requ, theano.tensor.TensorConstant):
            ...
            new_dims = [node.inputs[1][i] for i in range(self.ndim)]
...
def flatten(x, outdim=1):
    ...
    x_reshaped = theano.tensor.addbroadcast(
        x_reshaped, *[i for i in range(outdim) if broadcastable[i]])
    return x_reshaped
    ...
    if not isinstance(reps, (list, tuple)):
        ...
        # tensor.vector case
            ...
            if ndim is None:
                ...
                offset = theano.tensor.opt.assertTrue(offset, ge(offset, 0))
...
def _tensordot_as_dot(a, b, axes, dot, batched):
    ...
    # are last and the summed axes of b are first.
        ...
        other_axes = [[x for x in range(operand.ndim)
