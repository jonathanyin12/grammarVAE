...
class MoleculeVAE():
    ...
    def create(self,
        ...
        if weights_file:
            old_optimizer = load_model(weights_file, custom_objects={'vae_loss': vae_loss}).optimizer
            ...
            self.autoencoder.compile(optimizer=old_optimizer,
    ...
    def _buildEncoder(self, x, f, latent_rep_size, max_length, max_length_func, epsilon_std=0.01):
        ...
        def vae_loss(x, x_decoded_mean):
            if K.int_shape(x_decoded_mean)[1] == max_length:
                ...
                x = K.flatten(x)
                x_decoded_mean = K.flatten(x_decoded_mean)
                xent_loss = 100 * mse(x, x_decoded_mean)
            else:
